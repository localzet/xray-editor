//go:build js && wasm
// +build js,wasm

package main

import (
	"bytes"
	_ "embed"
	"encoding/json"
	"fmt"
	"io"
	"strings"
	"syscall/js"

	"github.com/xtls/xray-core/common/errors"
	"github.com/xtls/xray-core/common/platform/filesystem"
	"github.com/xtls/xray-core/core"
	"github.com/xtls/xray-core/infra/conf"
	json_reader "github.com/xtls/xray-core/infra/conf/json"
)

//go:embed assets/geoip.dat
var geoipRaw []byte

//go:embed assets/geosite.dat
var geositeRaw []byte

func main() {
	filesystem.NewFileReader = func(path string) (io.ReadCloser, error) {
		if strings.HasSuffix(path, "geoip.dat") {
			return io.NopCloser(bytes.NewReader(geoipRaw)), nil
		}

		if strings.HasSuffix(path, "geosite.dat") {
			return io.NopCloser(bytes.NewReader(geositeRaw)), nil
		}

		return nil, errors.New(path + " нельзя открыть в браузере")
	}

	js.Global().Set("XrayGetVersion", js.FuncOf(func(this js.Value, args []js.Value) any {
		return core.Version()
	}))

	js.Global().Set("XrayParseConfig", js.FuncOf(func(this js.Value, args []js.Value) any {
		if len(args) < 1 {
			fmt.Println("Некорректное количество аргументов")
			return nil
		}

		arg := args[0]
		if arg.Type() != js.TypeString {
			fmt.Println("Аргумент должен быть строкой")
			return nil
		}

		var configObj interface{}
		if err := json.Unmarshal([]byte(arg.String()), &configObj); err != nil {
			return err.Error()
		}

		replaceKeyFileAndCertificateFile(configObj)
		removeExtDatRules(configObj)

		modifiedConfigBytes, err := json.Marshal(configObj)
		if err != nil {
			return err.Error()
		}

		jsonConfig := &conf.Config{}
		jsonReader := &json_reader.Reader{
			Reader: bytes.NewReader(modifiedConfigBytes),
		}
		decoder := json.NewDecoder(jsonReader)

		if err := decoder.Decode(jsonConfig); err != nil {
			return err.Error()
		}

		if _, err := jsonConfig.Build(); err != nil {
			return err.Error()
		}

		return nil
	}))

	js.Global().Get("onWasmInitialized").Invoke()

	// Предотвращаем завершение программы.
	// Примечание: экспортированную функцию следует освобождать, если она больше не нужна,
	// и позволять программе завершиться. Для упрощения в этом примере это
	// опущено. Подробнее: https://pkg.go.dev/syscall/js#Func.Release
	select {}
}

func replaceKeyFileAndCertificateFile(obj interface{}) {
	switch v := obj.(type) {
	case map[string]interface{}:
		for key, value := range v {
			if key == "keyFile" {
				fmt.Printf("Заменяем keyFile: %v на моковый ключ\n", value)
				v["key"] = []string{`-----BEGIN PRIVATE KEY-----

7BHM8pzGDJJG/PiONMYk5fNj
-----END PRIVATE KEY-----
`}
				delete(v, "keyFile")
			} else if key == "certificateFile" {
				fmt.Printf("Заменяем certificateFile: %v на моковый сертификат\n", value)
				v["certificate"] = []string{`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`}
				delete(v, "certificateFile")
			} else {
				replaceKeyFileAndCertificateFile(value)
			}
		}
	case []interface{}:
		for _, item := range v {
			replaceKeyFileAndCertificateFile(item)
		}
	}
}

func removeExtDatRules(obj interface{}) {
	switch v := obj.(type) {
	case map[string]interface{}:
		fmt.Printf("Обрабатываем map с ключами: %v\n", getKeys(v))
		for key, value := range v {
			switch val := value.(type) {
			case []interface{}:
				fmt.Printf("Обрабатываем массив в ключе '%s' с %d элементами\n", key, len(val))
				filteredRules := make([]interface{}, 0)
				for _, rule := range val {
					switch r := rule.(type) {
					case string:
						if strings.Contains(r, "ext:") && strings.Contains(r, ".dat") {
							fmt.Printf("Удаляем строковое правило: %s\n", r)
							continue
						}
						filteredRules = append(filteredRules, r)
					case map[string]interface{}:
						if domains, ok := r["domain"].([]interface{}); ok {
							fmt.Printf("Обнаружены вложенные доменные правила с %d элементами\n", len(domains))
							filteredDomains := make([]interface{}, 0)
							for _, domain := range domains {
								if domainStr, ok := domain.(string); ok {
									if strings.Contains(domainStr, "ext:") && strings.Contains(domainStr, ".dat") {
										fmt.Printf("Удаляем вложенное правило: %s\n", domainStr)
										continue
									}
									filteredDomains = append(filteredDomains, domain)
								}
							}
							r["domain"] = filteredDomains
						}
						filteredRules = append(filteredRules, r)
					default:
						filteredRules = append(filteredRules, rule)
					}
				}
				v[key] = filteredRules
			case map[string]interface{}:
				removeExtDatRules(val)
			}
		}
	case []interface{}:
		for _, item := range v {
			removeExtDatRules(item)
		}
	}
}

func getKeys(m map[string]interface{}) []string {
	keys := make([]string, 0, len(m))
	for k := range m {
		keys = append(keys, k)
	}
	return keys
}
